...
@views function update_halo!(A, bufs, neighbors, comm)
    # x-dimension
    (neighbors.left  != MPI.PROC_NULL) && copyto!(bufs.send_left,  A[:, 2    ])
    (neighbors.right != MPI.PROC_NULL) && copyto!(bufs.send_right, A[:, end-1])
    ...
    (neighbors.left  != MPI.PROC_NULL) && copyto!(A[:, 1  ], bufs.recv_left)
    (neighbors.right != MPI.PROC_NULL) && copyto!(A[:, end], bufs.recv_right)

    # y-dimension
    (neighbors.up   != MPI.PROC_NULL) && copyto!(bufs.send_up, A[2    , :])
    (neighbors.down != MPI.PROC_NULL) && copyto!(bufs.send_down, A[end-1, :])
    ...
    (neighbors.up   != MPI.PROC_NULL) && copyto!(A[1  , :], bufs.recv_up)
    (neighbors.down != MPI.PROC_NULL) && copyto!(A[end, :], bufs.recv_down)
    return nothing
end
...
function run_diffusion(; ns=64, nt=1000, do_save=false)
    MPI.Init()
    comm      = MPI.COMM_WORLD
